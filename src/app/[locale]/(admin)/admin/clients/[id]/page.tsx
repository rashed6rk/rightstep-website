...
import { stepKeys } from "@/lib/portal";
import { deliverableKindIcon, deliverableKindTone } from "@/lib/portal";
import { buildAdminData, type AdminClient } from "@/lib/admin";
...
export async function generateMetadata({
  ...
  const client = buildAdminData().clients.find((c) => c.id === id);
  return {
    title: client ? `${client.company} — ${t("title")}` : t("title"),
...
export async function generateStaticParams() {
  return buildAdminData().clients.map((c) => ({ id: c.id }));
}
...
export default async function ClientDetailPage({
  ...
  const { clients, bookings, resources } = buildAdminData();
  ...
  return (
    <ClientDetailContent
      client={client}
      sessions={bookings.filter((b) => b.clientId === id)}
      files={resources.filter((r) => r.clientId === id)}
    />
  );
}
...
function ClientDetailContent({
  ...
  client: AdminClient;
  sessions: ReturnType<typeof buildAdminData>["bookings"];
  files: ReturnType<typeof buildAdminData>["resources"];
}) {
              ...
              {files.map((f) => (
                <li key={f.id} className="flex items-center gap-3 border-t border-line py-3 first:border-t-0">
                  ...
                  <a
                    href="#"
                    download
                    aria-label={tDeliverables("download")}
                    className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-line text-heading transition-colors hover:border-navy/40"
                  >
