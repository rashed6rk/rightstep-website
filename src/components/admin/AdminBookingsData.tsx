...
import { getAdminSessions, getAdminClients } from "@/lib/api";
...
export function AdminBookingsData() {
  ...
  useEffect(() => {
    Promise.all([getAdminSessions(), getAdminClients()])
      .then(([b, c]) => {
  ...
  const clientOf = (id: string | number) =>
